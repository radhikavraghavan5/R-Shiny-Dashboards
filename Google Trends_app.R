...
data("categories")
head(categories)
res <- gtrends(c("economy news","layoffs"), geo = c("US"), time = "all")
plot(res)
...
server <- function(input, output) {
  ...
  output$lineplot <- renderPlot({
    ...
    plot(x = selected_trends()$date, y = selected_trends()$close, type = "l",
         xlab = "Date", ylab = "Trend index", col = color, fg = color, col.lab = color, col.axis = color, lwd = 2)
    # Display only if smoother is checked
    if(input$smoother){
      smooth_curve <- lowess(x = as.numeric(selected_trends()$date), y = selected_trends()$close, f = input$f)
      lines(smooth_curve, col = "#FF0000", lwd = 3)
    }
  })
  ...
  output$desc <- renderText({
    ...
    paste0(trend_text, "The index is set to 1.0 on January 1, 2004 and is calculated only for US search traffic.")
  })
}
